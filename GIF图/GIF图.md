...
当系统提供的方式再一次不能用的时候，又需要我们开发者手动来实现了。怎么来实现呢？现在我们已经可以拿到任意一帧图片和每一帧图片播放的时长了，只要能按播放时长来切换每一帧图片不就实现GIF播放了吗！只需要一个定时器控制不就可以了。思路已经有了，是不是马上就想动手（写轮子）了？

![lunzi](./images/lunzi.jpg)
